...
from my_clip.model import build_model
from my_clip.model_LORA import build_model_LORA
from my_clip.tokenizer import SimpleTokenizer as _Tokenizer
...
def load(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", jit=False, is_train=False, pretrained=True, args=None):
    """Load a CLIP model
    Parameters
    ----------
    name : str
    ...
    """
    if args.lora_r != 0:
        build_model_func = build_model_LORA
    else:
    ...
    if not jit:
        try:
            model = build_model_func(state_dict or model.state_dict(), args=args).to(device)
        except KeyError:
            sd = {k[7:]: v for k,v in state_dict["state_dict"].items()}
            model = build_model_func(sd, args=args).to(device)
